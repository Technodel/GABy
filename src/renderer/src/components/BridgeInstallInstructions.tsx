...
export default function BridgeInstallInstructions({ autoCopy = false, previouslyConnected }: Props) {
  ...
  function buildSetupCommands(token: string): void {
    ...
    const winCmd = `@echo off\r\ntitle SUNy Bridge Setup\r\ncolor 0A\r\nset BRIDGE_DIR=%APPDATA%\\suny-bridge\r\nif not exist "%BRIDGE_DIR%" mkdir "%BRIDGE_DIR%"\r\necho ============================================\r\necho SUNy is preparing your local Bridge\r\necho ============================================\r\necho.\r\nif not exist "%BRIDGE_DIR%\\suny-bridge.exe" (\r\n  echo [1/3] Downloading SUNy Bridge...\r\n  echo       This can take up to 1 minute on slow internet.\r\n  powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri '${exeUrl}' -OutFile '%APPDATA%\\\\suny-bridge\\\\suny-bridge.exe' -UseBasicParsing"\r\n  if errorlevel 1 (\r\n    echo.\r\n    echo SUNy could not download the Bridge.\r\n    echo Please check your internet connection and try again.\r\n    pause\r\n    exit /b 1\r\n  )\r\n  echo [2/3] Download complete.\r\n) else (\r\n  echo [1/3] Bridge already installed.\r\n  echo [2/3] Reusing existing Bridge.\r\n)\r\necho [3/3] Starting Bridge in background...\r\npowershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process -FilePath '%APPDATA%\\\\suny-bridge\\\\suny-bridge.exe' -ArgumentList '--token ${token} --server ${serverUrl}' -WindowStyle Hidden"\r\nif errorlevel 1 (\r\n  echo.\r\n  echo SUNy could not start the Bridge.\r\n  echo Please run the installer again.\r\n  pause\r\n  exit /b 1\r\n)\r\necho.\r\necho SUNy Bridge started successfully.\r\necho You can close this window now.\r\ntimeout /t 2 >nul\r\n`;
    setCmd(c);
