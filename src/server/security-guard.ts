...
/**
 * SUNy Security Guard Ã¢â‚¬â€ Protected Files + Credential Scanner
 *
...
export function buildProtectedFileMessage(filePath: string): string {
  return (
    `Ã¢Å¡Â Ã¯Â¸Â File "${filePath}" is protected Ã¢â‚¬â€ it contains critical project configuration. ` +
...
export function isPathWithinProject(filePath: string, projectPath: string): boolean {
  // Use path.resolve() — handles Windows drive letters, UNC paths, and ..
  // traversal correctly on all platforms. The custom resolvePath() was
  // broken for Windows absolute paths like C:\foo\..\bar.
  const resolvedFile = path.resolve(filePath);
  const resolvedProject = path.resolve(projectPath);
  const separator = resolvedProject.endsWith(path.sep) ? resolvedProject : resolvedProject + path.sep;
  return resolvedFile.startsWith(separator) || resolvedFile === resolvedProject;
}
