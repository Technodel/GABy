/**
 * SUNy Error Auto-Correction â€” a `self_heal` tool that the AI can call
...
export async function analyzeError(
  ...
  try {
    ...
    return (result.text ?? '').trim() || '[No fix suggested â€” error not recognized]';
  } catch {
    return '[Error analysis failed â€” try again or report the issue]';
  }
}
